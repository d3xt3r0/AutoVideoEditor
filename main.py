...
def cut_video(loc, start, interval, clip_duration, output_name):
    ...
    try:
        ...
        final = clip.subclip(start, start+clip_duration)
...
if __name__ == "__main__":
    ...
    start = float(input("> Enter the start time [eg: 0.0]"))
    interval = int(input("> Enter the interval time [eg: 10 ie, every 10 seconds]"))
    clip_duration = int(input("> Enter the clip duration [eg: 2 ie, for 2 seconds]"))
